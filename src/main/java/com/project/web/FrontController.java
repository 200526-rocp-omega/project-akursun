		...
		try {
			...
			case "accounts": {
				...
				if(portions.length == 1) {
					...
					List<Account> a = accountController.findAll();
					res.setStatus(200);
					res.getWriter().println(om.writeValueAsString(a));
				}
...
/* TO-DO The doPut method below is still IN PROGRESS
...
//		res.setContentType("application/json");
//		
//		String URI = req.getRequestURI().replace("/rocp-project", "").replaceFirst("/", "");
//		String[] portions = URI.split("/");
//		HttpSession session = req.getSession();
//		res.setStatus(404);
//		
